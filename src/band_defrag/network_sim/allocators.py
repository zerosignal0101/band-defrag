...
def ksp_allocate_service(
    ...
    for k, path in enumerate(ksp_paths):
        ...
        available_bands = get_available_bands(edge_key_list, allocation_status)
        ...
        for wave_id in free_bands:
            ...
            for edge_key in edge_key_list:
                allocation_status[edge_key][wave_id] = True
                allocated_service_idx[edge_key][wave_id] = service.service_id
...
def release_service(
        allocation_status: Dict[Tuple[int, int], NDArray[bool]],  # RW
        allocated_service_idx: Dict[Tuple[int, int], NDArray[int]],  # RW
        edge_distances: Dict[Tuple[int, int], float],  # RO (not directly used for release but part of context)
