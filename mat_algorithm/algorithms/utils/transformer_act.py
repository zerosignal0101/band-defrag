...
# def discrete_autoregreesive_act(decoder,
#                                 obs_rep, obs,
...
#     自回归采样动作（推理时用）
#     ------------------------------------------------------------------
#     pad_mask : (B, N)  1→该 agent 存在；0→padding
#     """
#     device = tpdv["device"]
#     shifted_action = torch.zeros(batch_size, n_agent, action_dim + 1, **tpdv)
#     shifted_action[:, 0, 0] = 1.        # <START>
#     output_action = torch.zeros(batch_size, n_agent, 1, device=device, dtype=torch.long)
#     output_action_log = torch.zeros_like(output_action, dtype=torch.float32)
#     if pad_mask is None:                        # 全部都是真实 agent
#         pad_mask = torch.ones(batch_size, n_agent, dtype=torch.bool, **tpdv)
#
#     for i in range(n_agent):
#         if pad_mask[:, i].sum() == 0:           # 整个 batch 的第 i 列全是 padding
#             continue
#         logit = decoder(shifted_action, obs_rep, obs, pad_mask)[:, i, :]       # (B, A)
#         if available_actions is not None:
#             logit[available_actions[:, i] == 0] = -1e10
#         logit[~pad_mask[:, i]] = -1e10                                # 完全 mask 掉 padding 行
#
...
#         # 只回填有效行
#         valid = pad_mask[:, i]
#         output_action[valid,  i, 0] = act[valid]
#         output_action_log[valid, i, 0] = logp[valid]
#
#         # 准备给下一位 agent
#         if i + 1 < n_agent:
#             shifted_action[valid, i + 1, 1:] = F.one_hot(act[valid], num_classes=action_dim).float()
#
